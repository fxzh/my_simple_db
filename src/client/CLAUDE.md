...
# 启动参数
./client [-p 端口号] [-h 主机地址] [-c SQL文本] [-a]    缺省端口 8123、主机 127.0.0.1；取值参数支持黏连写法
...
-a 无值参数仅支持独立形式：置位后逐行原样回显非空输入行(含注释行)，并跳过"已发送消息"前缀行
...
输入未到 ';' 时只积累不发送。Ok 帧解码命令标签打印(CREATE/DROP, INSERT/DELETE 附行数, 空语句无输出)，Error 帧补 "ERROR: " 前缀打印，
