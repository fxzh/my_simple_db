...
# 文件
types.h         公共类型：PageId(table_id<<32|page_no)、ColType、Value(variant)、TableMeta、Row
...
codec.h/.cpp    记录[长度 u16][列数据]序列化；类型名解析 int/bigint/float/double/char(n)/varchar(n)
...
- 记录 ≤4068B，无 NULL，表最多约 4000B/行；跨页记录不支持(将来 M3 树内处理)
