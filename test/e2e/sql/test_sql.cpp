...
static void print_diff(const std::vector<std::string>& exp, const std::vector<std::string>& act)
{
...
int main(int argc, char* argv[])
{
    ...
    std::cerr << "golden diff 不一致: " << argv[1] << std::endl;
    if (r.timed_out) {
    ...
    std::cerr << "-- 期望(-) / 实际(+) --" << std::endl;
    print_diff(split_lines(expected), split_lines(r.out));
